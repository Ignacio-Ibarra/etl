...
df_output <- bind_rows(pbisectores_fnys,
                            pbisectores_indec) %>% 
  distinct(anio, participacion_agro = agricultura_ganaderia_caza_y_silvicultura)

df_anterior <- argendataR::descargar_output(nombre = output_name,
                                            subtopico = subtopico,
                                            entrega_subtopico = "primera_entrega") %>% 
  mutate(
    participacion_agro = 100*participacion_agro
  )
...
etiquetas_nuevas <- data.frame(
  ...
  descripcion = c("Año de referencia",
                  "Participación del sector 'Agricultura, caza y silvicultura' en el VAB a precios básicos en pesos corrientes")
)
...
aclaracion <- paste0("Se modificó el calculo realizado cambiando la fuente de información. ",
                     "De Fundación Norte y Sur, Cuentas Nacionales, se tomó la participación del sector 'Agricultura, caza y silvicultura', ",
                     ...
                     "Los valores de la variables indicador se pasaron de proporción a porcentaje. "
...
df_output %>%
  argendataR::write_output(
    ...
    unidades = list("participacion_agro" = "porcentaje"),
