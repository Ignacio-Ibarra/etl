...
subtopico <- "AGROPE"
output_name <- "produccion_carne_tipos_arg_evo"
...
df_output <- df_fao %>% 
  dplyr::filter(element == "Production", item %in% names(carnes), iso3 == "ARG") %>% 
  mutate(tipo_carne = carnes[item]) %>% 
  select(anio = year, tipo_carne, valor = value)
...
comparacion <- argendataR::comparar_outputs(
  ...
  df = df_output,
  nombre = output_name,
  pk = c('tipo_carne','anio'), # variables pk del dataset para hacer el join entre bases
  drop_joined_df =  F
)
...
df_output %>%
  argendataR::write_output(
    ...
    analista = analista,
    pk = c("tipo_carne",'anio'),
    es_serie_tiempo = T,
    control = comparacion, 
    columna_indice_tiempo = 'anio',
    descripcion_columnas = descripcion,
    unidades = list("valor" = "toneladas"),
    aclaraciones = "Los datos de carne de cerdo entre 2004 y 2007 difieren levemente de los datos actuales."
  )