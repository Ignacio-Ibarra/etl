...
subtopico <- "AGROPE"
output_name <- "produccion_global_carne_bovina_ultimo_anio_disponible"
output_name_old <- "produccion_global_carne_bovina_2021"
...
df_output <- df_carnes %>%
  dplyr::filter(item %in% carne_vacuna, anio == max(anio)) %>% 
...
df_comparable <- df_carnes %>%
  dplyr::filter(item %in% carne_vacuna, anio == (max(anio) - 1)) %>% 
  group_by(iso3, pais) %>% #supongo que dentro de lo que es carne vacuna, no va a diferir el ultimo año disponible
  summarise(
    valor = sum(valor, na.rm = T)
  ) %>% 
