...
subtopico <- "AGROPE"
output_name <- "produccion_global_carne_bovina_evo"
...
carne_vacuna <- c("Meat of buffalo, fresh or chilled", 
                  "Meat of cattle with the bone, fresh or chilled")

df_output <- df_carnes %>%
  dplyr::filter(item %in% carne_vacuna) %>% 
  group_by(anio, iso3, pais) %>% #supongo que dentro de lo que es carne vacuna, no va a diferir el ultimo año disponible
  summarise(
