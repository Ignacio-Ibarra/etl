...
subtopico <- "INDUST"
output_name <- "composicion_importaciones_intensidad_origen.csv"
...
query_output <- glue::glue(
  "SELECT t as anio, c.i as exporter_code, p.country_iso3 as exporter_iso3, c.j as importer_code, h.lall_code, SUM(c.v) as impo
   FROM comercio as c
   LEFT JOIN hs_to_lall as h ON h.hs02 = c.k
   LEFT JOIN paises as p ON c.i = p.country_code
   where c.j = 32
   GROUP BY t, c.i, p.country_iso3, c.j, h.lall_code"
)
...
pks <- c("anio", "exporter_iso3", "geonombreFundar", "clasificacion_lall")
