...
# Metadatos 
subtopico <- "INDUST"
output_name <- "pib_industrial_por_regiones.csv"
...
df_break_manuf_regiones <- df_break_manuf %>% 
  ...
  mutate(region = case_when(subregion_unsd == 'América Latina y el Caribe' ~ 'América Latina',
                            geocodigoFundar == 'TUR' ~ "Asia",
                            continente == 'Europa' ~ 'Europa',
                            continente == 'África' ~ 'África',
                            geocodigoFundar == 'USA' ~ 'Estados Unidos',
                            continente == 'Asia' ~ 'Asia',
                            TRUE ~ 'Otros'))  %>% 
  group_by(anio, region) %>% 
  summarise(gdp_indust = sum(gdp_indust, na.rm = T)) %>% 
  ungroup()


df_output <- df_break_manuf_regiones %>% 
  group_by(anio) %>% 
  mutate(prop_industry_gdp = 100 * gdp_indust / sum(gdp_indust, na.rm = T)) %>% 
  ungroup() %>% 
  select(anio, region, industry_gdp = gdp_indust, prop_industry_gdp)
...
df_output %>%
  argendataR::write_output(
    ...
    unidad = list("industry_gdp" = "dólares", "prop_industry_gdp" = "porcentaje"))
