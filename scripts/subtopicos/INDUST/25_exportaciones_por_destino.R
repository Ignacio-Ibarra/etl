...
subtopico <- "INDUST"
output_name <- "exportaciones_por_destino.csv"
...
query_output <- glue::glue(
  "SELECT t as anio, c.i as exporter_code, c.j as importer_code, p.country_iso3 as importer_iso3, h.tipo_bien, SUM(c.v) as expo
   FROM comercio as c
   LEFT JOIN hs_to_lall as h ON h.hs02 = c.k
   LEFT JOIN paises as p ON c.j = p.country_code
   where c.i = 32
   GROUP BY t, c.i, c.j, p.country_iso3, h.tipo_bien"
)
...
df_output <- df_query %>% 
  ...
  ungroup() %>% 
  select(anio, importer_iso3, geonombreFundar, tipo_bien, expo, prop)

rm(df_query)
gc()

pks <- c("anio", "importer_iso3", "geonombreFundar", "tipo_bien")
