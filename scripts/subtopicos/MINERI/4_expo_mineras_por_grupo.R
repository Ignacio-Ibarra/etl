...
subtopico <- "MINERI"
output_name <- "expo_mineras_por_grupo"
analista = "Kevin Corfield"
fuente1 <- "R268C137" # SIACAM


df_siacam <- arrow::read_parquet(argendataR::get_clean_path(fuente1)) 

metales <- c("Oro", "Plata", "Cobre", "Litio")
...
otros <- df_siacam %>% 
  ...
  ungroup() %>% 
  arrange(-fob) %>% 
  mutate(cumulative_share = cumsum(fob) / sum(fob)) %>% 
...
df_output <- df_siacam %>% 
  ...
  summarise(expo_grupo = sum(fob, na.rm = T)) %>% 
  arrange(anio, -expo_grupo)
...
etiquetas_nuevas <- data.frame(
  ...
  descripcion = c("Clasificación por grupo de minerales",
                  "Exportaciones en dólares estadounidenses, en precios corrientes")
)
...
df_output %>%
  argendataR::write_output(
    ...
    unidades = list("expo_grupo" = "unidades"),
