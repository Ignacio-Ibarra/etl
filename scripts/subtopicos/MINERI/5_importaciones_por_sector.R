...
df_output <- df_siacam %>%
  ...
  summarise(cif_siacam = round(sum(cif, na.rm=T))) %>% 
  ungroup() %>% 
  left_join(df_ica, join_by(anio)) %>% 
  mutate(importaciones_sector_perc = round(100*cif_siacam / cif_total, 5)) %>% 
  select(anio, sector, importaciones_sector_perc) %>% 
  complete(anio, sector, fill = list(importaciones_sector_perc = 0)) %>% 
  arrange(sector, anio)
...
etiquetas_nuevas <- data.frame(
  variable_nombre = c("sector",
                      "importaciones_sector_perc"),
  descripcion = c("Clasificación por sector de minerales",
                  "Participación en las importaciones de toda la economía")
)
...
df_output %>%
  argendataR::write_output(
    ...
    unidades = list("importaciones_sector_perc" = "porcentaje")
