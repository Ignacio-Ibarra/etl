...
subtopico <- "SEBACO"
output_name <- "18_expo_impo_saldo_x_sector"
...
# Armar total SBC
data_sbc <- data_sectores %>% 
  mutate(sector = "SBC") %>% 
  group_by(anio, sector) %>% 
  summarize(balanza = sum(balanza),
            expo = sum(expo),
            impo = sum(impo)) %>% 
  ungroup()

df_output <- data_sectores %>% 
...
etiquetas_nuevas <- data.frame(
  ...
  descripcion = c("Rama de actividad perteneciente a 'Sectores basados en conocimiento' (SBC)",
                  "Valor de las exportaciones en millones de dólares corrientes",
                  "Valor de las importaciones en millones de dólares corrientes",
...
df_output %>%
  argendataR::write_output(
    ...
    unidades = list("expo" = "millones de dólares corrientes",
                    "impo" = "millones de dólares corrientes",
                    "balanza" = "millones de dólares corrientes")
  )
