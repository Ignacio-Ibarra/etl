...
subtopico <- "SEBACO"
output_name <- "20_composicion_expo_sbc"
...
df_output <- data_sectores %>% 
  select(anio, sector = descripcion, expo) %>% 
  group_by(anio) %>% 
  mutate(
    composicion_expo = expo / sum(expo)
  ) %>% 
  ungroup() %>% 
  select(-expo)
...
df_anterior <- argendataR::descargar_output(nombre = output_name,
  ...
  rename(
    composicion_expo = prop_sobre_sbc_expo,
    sector = descripcion
  ) %>% 
...
etiquetas_nuevas <- data.frame(
  ...
  descripcion = c("Rama de actividad perteneciente a 'Sectores basados en conocimiento' (SBC)",
                  "Proporción de la rama sobre el total exportado de SBC")
)
...
df_output %>%
  argendataR::write_output(
    ...
    cambio_nombre_cols = list('composicion_expo' = 'prop_sobre_sbc_expo',
                              'sector' = 'descripcion'),
    unidades = list("composicion_expo" = "proporcion")
  )
