...
output_name <- "12_tasa_de_inflacion_mensual_argentina_ene1989_dic1993.csv"
...
ipc_gba <- ipc_gba %>%
  ...
  mutate(var_ipc_mes = (nivel_general/lag(nivel_general, 1)-1)*100) %>% 
  filter( fecha >= lubridate::ym("1989-1") & fecha <= lubridate::ym("1993-12")) %>% 
  select(-c(mes, anio, nivel_general)) 

df_output <- ipc_gba %>%
  rename(inflacion_mensual = var_ipc_mes)
