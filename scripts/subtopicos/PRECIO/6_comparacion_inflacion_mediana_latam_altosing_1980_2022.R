...
output_name <- "6_comparacion_inflacion_mediana_latam_altosing_1980_2022.csv"
...
ipc_fyns <-  ipc_fyns %>% 
  filter(anio %in% 1980: 2006 | anio %in% 2017:2018) %>% 
...
ipc_weo <- ipc_weo %>% 
  filter(iso3 %in% paises_sel & anio %in% 1980:max(df_ipc$anio) &
...
geonomenclador <- get_nomenclador_geografico()

df_ipc <- left_join(df_ipc, geonomenclador, by = c("iso3" = "codigo_fundar"))

df_ipc <- df_ipc %>% 
  mutate(grupo_de_paises = case_when(
    (str_detect(tolower(subregion_unsd), "europa") & (grupos_ingreso_bm_es == "Ingreso Alto")) |
      iso3 %in% c("USA", "CAN", "AUS", "NZL", "JPN", "KOR",
                  "HKG", "SGP", "TWN", "ISR", "ARE",  "QAT", "SAU", "KWT", "OMN"   ) ~ "Países de Altos Ingresos",
    iso3 %in% amlat_iso3 ~ "América Latina y Caribe",
    T ~ NA
  )) %>% 
  filter(!is.na(grupo_de_paises))

df_ipc <- df_ipc %>% 
  group_by(grupo_de_paises, anio) %>% 
