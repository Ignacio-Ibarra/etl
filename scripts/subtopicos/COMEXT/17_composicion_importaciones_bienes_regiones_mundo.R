...
df_output <-  partners %>%
  ...
  select(year, iso3, location_name_short_en, partner_region_id, partner_region_name_short_es, import_value_pc) %>% 
  collect()
...
df_output %>%
  argendataR::write_output(directorio = 'data/COMEXT/',
                           ...
                           etiquetas_indicadores = list("import_value_pc" = "Importaciones de bienes (% del total exportado en bienes)"),
