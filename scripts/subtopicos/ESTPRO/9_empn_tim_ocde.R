...
df_anterior <- argendataR::descargar_output(nombre =output_name, subtopico = subtopico, entrega_subtopico = "primera_entrega") %>% 
  left_join(icio_tables, join_by(ind_tim == old_code)) %>% 
  select(-ind_tim) %>% 
  rename(tim_code = code,
         intensidad_ocde = intensidad_id_ocde, 
         tim_code_desc = ind_tim_desc_es)%>% 
  mutate(variable = "Cantidad de personas o de puestos de trabajo, en miles") %>% 
  mutate(iso3 = recode(iso3, !!!reemplazos_codigos)) %>% 
  left_join(geonomenclador, join_by(iso3)) %>% 
...
comparacion <- argendataR::comparar_outputs(
  ...
  df = df_output,
  pk = c("anio","iso3", "tim_code"), # variables pk del dataset para hacer el join entre bases
  drop_joined_df =  F
)
