...
subtopico <- "ESTPRO"
output_name <- "vab_sectorial_granregion"
...
df_output <- df_cepal %>% 
 ...
 mutate(
  ...
  drop_na(letra) %>% 
  group_by(anio, gran_region, letra, letra_desc_abrev) %>% 
  summarise(
    vab_pb = sum(vab_pb, na.rm = T)
  ) %>% 
  ungroup() %>% 
  group_by(anio, gran_region) %>% 
  mutate(
...
df_anterior <- argendataR::descargar_output(nombre =output_name, subtopico = subtopico, entrega_subtopico = "primera_entrega") %>% 
  rename(gran_region = gran_region_desc) %>% select(-gran_region_id)

comparacion <- argendataR::comparar_outputs(
  df_anterior = df_anterior,
  df = df_output,
  pk = c("anio","letra", "gran_region"), # variables pk del dataset para hacer el join entre bases
  drop_joined_df =  F
)
