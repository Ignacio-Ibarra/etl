...
subtopico <- "ESTPRO"
output_name <- "particip_sectorial_empleoyvab"
...
################################################################
#           Share puestos: Fichas Sectoriales + CGI Sexo y Edad
...
#############################################################################
#          Share VAB: CGI (VAB por sector Cuadro 4) + Fichas Sectoriales    #
#############################################################################


df_vab <-  arrow::read_parquet(get_clean_path(fuente5)) %>% 
  dplyr::filter(trimestre == "Total" & sub_sector == "Total sector") %>% 
  mutate(
    letra = case_when(
      letra %in% c("A","B") ~ "A_B",
      letra %in% c("L","Q") ~ "L_Q",
  ...
  ungroup() %>% 
  group_by(anio) %>% 
  mutate(share_vab = vab_pb / sum(vab_pb, na.rm = T)) %>% 
  ungroup() 


#############################################################################
#   merge por anio y letra
#############################################################################
...
df_output <- df_puestos_total %>% 
  ...
  group_by(anio) %>% 
  mutate(share_puestos = puestos / sum(puestos, na.rm=T)) %>% 
  ungroup() %>% 
  left_join(df_vab, join_by(anio, letra)) %>% 
  select(anio, letra, letra_desc_abrev, id_tipo_sector, tipo_sector, puestos, share_puestos, vab_pb, share_vab)
...
df_anterior <- argendataR::descargar_output(nombre = output_name, subtopico = subtopico, entrega_subtopico = "primera_entrega") %>% 
  rename(share_puestos = share_sectorial) %>% 
  mutate(letra = case_when(
    letra == "AB" ~ "A_B",
    letra == "NO" ~ "N_O",
    letra == "LQ" ~ "L_Q",
...
df_output %>%
  argendataR::write_output(
    ...
    cambio_nombre_cols = list("share_puestos" = "share_sectorial"),
    unidades = list("share_puestos" = "proporción", 
                    "puestos" = "unidades",
                    "share_vab" = "proporcion",
                    "vab_pb" = "en millones de pesos a precios corrientes")
  )
