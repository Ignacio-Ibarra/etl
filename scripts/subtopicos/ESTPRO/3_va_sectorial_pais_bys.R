...
subtopico <- "ESTPRO"
output_name <- "va_sectorial_pais_bys"
...
df_output <- arrow::read_parquet(get_clean_path(fuente1)) %>% 
  dplyr::filter(tipo_sector == "Servicios") %>% 
  group_by(anio, iso3, pais_nombre) %>% 
  summarise(particip_servicios_pib = sum(share_vab)/100)
  

df_anterior <- argendataR::descargar_output(nombre = output_name, subtopico = subtopico, entrega_subtopico = "primera_entrega") %>% 
  dplyr::filter(!(is.na(particip_servicios_pib)))
...
etiquetas_nuevas <- data.frame(
  variable_nombre = c("pais_nombre",
                      "particip_servicios_pib"),
  descripcion = c("Nombre de país",
                  "Participación de los servicios en PIB, en proporción")
)
