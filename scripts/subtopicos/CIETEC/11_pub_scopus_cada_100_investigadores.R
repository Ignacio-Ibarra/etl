...
# Defino variables
subtopico <- "CIETEC"
output_name <- "11_pub_scopus_cada_100_investigadores.csv"
analista <- "Ignacio Ibarra"

# Defino las fuentes
fuente1 <- 'R349C224' # RICyT Publicaciones en SCOPUS cada 100 investigadores
...
plot_data <- df_output %>% 
  mutate(pais_label = factor(stringr::str_pad(pais, width = 10), levels = rev(stringr::str_pad(unique(pais), width = 10)))) %>% 
