...
################################
# SERIE AMÉRICAS (modificada por Argentina)
  

df_americas <- bind_rows(
  df_unwto_complete_regiones,
  df_owid_complete %>% 
    dplyr::filter(anio<2016)
  ) %>% 
  dplyr::filter(geonombreFundar == "Américas") %>% 
  group_by(anio) %>% 
  summarise(
    arribos_turisticos_internacionales_americas = 1000000 * sum(arribos_turisticos_internacionales_millon, na.rm = T)
  )  %>%
  left_join(df_serie_argentina_unwto_empalme, join_by(anio)) %>% 
  left_join(df_serie_argentina %>% 
              select(anio, arribos_turistas), join_by(anio)) %>% 
  drop_na(arribos_turistas) %>% 
  mutate(
    int_tourism_arraivals_americas_corregido = (arribos_turisticos_internacionales_americas + arribos_turistas - int_tourism_arraivals_unwto_empalme_arg)/1000000
  ) %>% 
  select(anio, int_tourism_arraivals_americas_corregido)
...
df_output <- df_serie_argentina %>% 
  left_join(df_mundo, join_by(anio)) %>% 
  left_join(df_americas, join_by(anio)) %>% 
  mutate(
    share_argentina_mundo = 100 * (arribos_turistas/1000000) / int_tourism_arraivals_wld_corregido,
    share_argentina_americas = 100 * (arribos_turistas/1000000) / int_tourism_arraivals_americas_corregido
  ) %>% 
  select(anio, 
         `Turismo receptivo en Argentina, como % del turismo mundial` = share_argentina_mundo, 
         `Turismo receptivo en Argentina, como % del turismo de las Américas` = share_argentina_americas) %>% 
  pivot_longer(
    -anio, names_to = "variable", values_to = "share_turismo_receptivo"
  ) %>% 
  drop_na(share_turismo_receptivo)
