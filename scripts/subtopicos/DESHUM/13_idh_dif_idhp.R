...
subtopico <- "DESHUM"
output_name <- "idh_dif_idhp"
analista = "Pablo Sonzogni"
fuente1 <- "R290C159" # diff_hdi_phdi 
fuente2 <- "R282C151" # hdi
...
df_output <- df_all %>% 
  ...
  mutate(
    ...
    nivel_agregacion = ifelse(is.na(nivel_agregacion), "agregacion", "pais")
  ) %>% 
  rename(idh = hdi_value,
         dif_idh_idhp = diff_hdi_phdi)
