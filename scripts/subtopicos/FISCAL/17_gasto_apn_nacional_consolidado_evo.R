...
# Defino variables
subtopico <- "FISCAL"
output_name <- "gasto_apn_nacional_consolidado_evo.csv"
...
fuente3 <- 'R326C201' # Nacional
fuente4 <- 'R325C200' # Consolidado
...
# Nacional
df_nac <- argendataR::get_clean_path(fuente3) %>% 
  arrow::read_parquet() %>% 
  dplyr::filter(codigo == "1.0") %>% 
  select(anio, `Nacional` = valores)

# Consolidado
df_cons <- argendataR::get_clean_path(fuente4) %>% 
  arrow::read_parquet() %>% 
  dplyr::filter(codigo == "1.0") %>% 
  select(anio, `Consolidado` = valores)

df_output <- df_apn %>% 
  inner_join(df_pib, join_by(anio)) %>% 
  mutate(`APN` = 100 * total_credito_devengado / pbi,
         ) %>%
  select(anio, `APN`) %>% 
  left_join(df_nac, join_by(anio)) %>% 
  left_join(df_cons, join_by(anio)) %>% 
  pivot_longer(., -anio, names_to = "tipo_gasto", values_to = "porcentaje_pbi") %>% 
...
df_output %>%
  argendataR::write_output(
    ...
    unidades = list("porcentaje_pbi" = "porcentaje")
  )
