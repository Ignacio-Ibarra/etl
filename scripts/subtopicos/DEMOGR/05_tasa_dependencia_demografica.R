...
# Defino variables
subtopico <- "DEMOGR"
output_name <- "tasa_dependencia_demografica.csv"
...
df_censos <- df_indec %>%
  mutate(productiva_bool = case_when(
    as.numeric(str_extract(edad, "(?<=-)[0-9]+")) < 20 ~ "no_productiva",
    as.numeric(str_extract(edad, "(?<=-)[0-9]+")) %>% 
      between(., 20, 64) ~ "productiva",
    as.numeric(str_extract(edad, "(?<=-)[0-9]+")) > 64 | edad == "85+" ~ "no_productiva")) %>% 
  group_by(anio = censo, productiva_bool) %>% 
...
# ggplot(df_output %>% dplyr::filter(anio<=2025), aes(x=anio, y = tasa_dependencia)) +
#   geom_line() +
#   theme_minimal() + 
#   ylim(0, NA)
