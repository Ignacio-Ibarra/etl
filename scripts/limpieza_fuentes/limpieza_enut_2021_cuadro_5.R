# Codigo de limpieza de datos de Encuesta Nacional de Uso del Tiempo 2021 Cuadro 5
...
enut_df_clean <-  enut_df %>% 
  ...
  pivot_longer(., !any_of(c("sexo", "nivel_ed")), names_to = "tipo_trabajo", values_to = "participacion") %>% 
  mutate(participacion  = as.double(replace(participacion, participacion == "/// (1)", NA))/100)
...
actualizar_fuente_clean(id_fuente_clean = 19,
                        dir = tempdir())