...
clean_tabla <- function(sheet_name, filas_columnas, skip, names_to, values_to){
  ...
  names(sheet_data) <- c('anio', cols$concatenado[-1])
...
sheet_name <- "expindices_04"
filas_columnas <- 3:4
skip <- 4
names_to <- c("variable","tipo_indice")
values_to <- "valor"
...
clean_filename <- glue::glue("{nombre_archivo_raw}_todas_las_columnas.parquet")

clean_title <- glue::glue("{titulo.raw} - Todas las columnas")
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c("anio", "variable", "tipo_indice")
)
