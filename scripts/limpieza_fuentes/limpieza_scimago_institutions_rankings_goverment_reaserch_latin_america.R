...
df_clean <- df_raw %>% 
  janitor::clean_names() %>% 
  rename(iso3 = country, anio = year) %>% 
  left_join(geonomenclador, join_by(iso3)) %>% 
  select(anio, iso3, nombre_pais, sector, institucion = institution, ranking = rank, ranking_global = global_rank)
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c('iso3','anio','sector','institucion')
)
