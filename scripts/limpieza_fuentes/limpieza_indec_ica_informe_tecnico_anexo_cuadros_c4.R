...
clean_sheet <- function(sheet_name, skip, filas_columnas, names_to, values_to){
  ...
  str_title <- readxl::read_excel(argendataR::get_raw_path(fuente_raw), 
                                  sheet = sheet_name, range = "A1",
                                  col_names = F) %>% pull()
  ...
  cols <- c('gran_rubro', 'rubro', cols$concatenado[-1])
  ...
  df <- sheet_data %>% 
    ...
    fill(gran_rubro, .direction = "down") %>%
    drop_na(rubro) %>% 
    pivot_longer(cols = !c(gran_rubro, rubro), 
                 names_to = names_to,
                 names_sep = "#",
                 values_to = values_to, 
...
skip = 5
filas_columnas = 3:5
names_to = c('periodo_considerado','anio','unidad_medida') 
values_to = 'expo'
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c('rubro', 'periodo_considerado', 'anio')
)
