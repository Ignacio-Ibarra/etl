...
df_clean <- df_stage %>% 
  select(country_id = countryCode, 
         country_en = countryName, 
         anio = fiscalYear, 
         indicator_name = serieName, 
         sector_id = itemId, 
         sector_name = itemName, 
         value = observationValue, 
         note = observationNote,
         unit) %>% 
  left_join(geonomneclador, join_by(country_id == m49_code_unsd)) %>% 
  mutate(
    iso3 = ifelse(country_id == 1, "WLD", iso3),
    pais_nombre = ifelse(country_id == 1, "Mundo", pais_nombre),
  ) %>% 
  select(iso3, pais_nombre, country_en, anio, indicator_name, sector_id, sector_name, value, note, unit)
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c('anio', 'iso3', 'indicator_name', 'sector_id')
)
