# Codigo de limpieza de datos de Encuesta Nacional de Uso del Tiempo 2021 Cuadro 14
...
valores <- as.vector(apply(enut_df, 1, c))

tipo_trabajos <- c("Doméstico", "Cuidados a otros miembros del hogar", "Apoyo a otros hogares, para la comunidad y voluntario")

medidas <- c("participacion", "proporcion_dia")

sexo <- c("Total", "Mujeres", "Varones")

enut_df_clean <- expand.grid(sexo, tipo_trabajos, medidas)
colnames(enut_df_clean) <- c('sexo', 'tipo_trabajos', 'medidas')
enut_df_clean<- enut_df_clean %>% arrange(sexo, tipo_trabajos) 
enut_df_clean$valores <- valores

enut_df_clean <- enut_df_clean%>% pivot_wider(names_from = "medidas", values_from = "valores") %>% 
  mutate( participacion = as.double(participacion)/100,
          proporcion_dia = as.double(proporcion_dia),
          minutos_dia = proporcion_dia * 24 * 60)
