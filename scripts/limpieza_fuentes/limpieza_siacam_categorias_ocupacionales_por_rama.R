...
df_clean <- read.csv(tmp_file, sep=";", na.strings = c("NA", "#N/A"), dec = ",", header = F, skip = 1) 

names(df_clean) <- c("rama_de_actividad","Asalariado registrado","Asalariado no registrado", "No registrado")


df_clean <- df_clean %>% 
  pivot_longer(-all_of("rama_de_actividad"),
               names_to = "categoria_ocupacional",
               values_to = "promedio_share")
