...
notas_col <- df_raw %>% colnames() %>% purrr::keep(., ~all(grepl("y\\d+n$",.x)))

flags_col <- df_raw %>% colnames() %>% purrr::keep(., ~all(grepl("y\\d+f$",.x)))
...
df_clean_notas <- df_raw %>% 
  select(all_of(c(id_cols, notas_col))) %>% 
  pivot_longer(!all_of(id_cols),
               names_to = 'year',
               names_transform = anios_transform,
               values_to = "notes")  
...
df_clean_landing <- df_clean_valores %>% 
  left_join(df_clean_notas, by = pks) %>% 
  left_join(df_clean_flags, by = pks) %>% 
  mutate(
    area_code_m49 = as.integer(str_remove(area_code_m49, "^\\'")),
    item_code_sdg = str_remove(item_code_sdg, "^\\'")
  ) 


rm(df_clean_flags, df_clean_notas, df_clean_valores)
...
df_clean <- df_clean_landing %>% 
  left_join(codigos, join_by(area_code_m49)) %>% 
  select(area_code_m49, area, iso3, pais_es = pais, year, item_code, item_code_sdg, item, element_code, element, unit, value, flags, notes) %>% 
  drop_na(value)
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c('area_code_m49', 'area', 'iso3', 'year', 'item_code', 
                                            'item_code_sdg', 'element_code', 
                                            'unit')
)
