...
df_clean_landing <- df_clean_valores %>% 
  ...
  mutate(
    ...
    item_code_cpc = str_remove(item_code_cpc, "^\\'")
  ) 
...
df_clean <- df_clean_landing %>% 
  ...
  select(iso3, pais, year, item_code, item_code_cpc, item, element_code, element, unit, value, flags, notes)
