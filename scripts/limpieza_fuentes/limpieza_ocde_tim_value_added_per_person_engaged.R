...
df_clean <- df_raw %>% 
  select(`REF_AREA: Reference area`, `ACTIVITY: Economic activity`, anio = `TIME_PERIOD: Time period`, valor_usd_por_emp = OBS_VALUE) %>% 
  ...
  select(anio, iso3, codigo_act, desc_act, valor_usd_por_emp)
...
actualizar_fuente_clean(id_fuente_clean = 104, 
                        path_clean = clean_filename,
                        nombre = clean_title, 
                        script = code_name)
