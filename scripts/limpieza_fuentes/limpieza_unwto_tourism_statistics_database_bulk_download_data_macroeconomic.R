...
filepath <- zip_file %>% 
  ...
  dplyr::filter(grepl("Macroeconomic.*\\.xlsx", Name)) %>% 
  pull(Name) 
...
df_clean <- extracted_path %>% 
  readxl::read_excel(., sheet = "SDG 8.9.1") %>% 
  janitor::clean_names() %>% 
  select(-index) %>% 
...
indicator <- df_clean$series_description %>% unique()
...
comparacion <- comparar_fuente_clean(df_clean,
                                     df_clean_anterior,
                                     pk = c('geo_area_code', 'sdg_indicator', 'time_period')
)
