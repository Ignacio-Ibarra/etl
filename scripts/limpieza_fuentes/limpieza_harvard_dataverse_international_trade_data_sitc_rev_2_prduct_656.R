...
df_clean <- df_raw %>% 
  mutate(
    ...
    country_id = as.integer(str_remove(partnerCountryId, "country-"))
         ) %>% 
  select(country_id, product_id, product_level = productLevel, year, import_value = exportValue, export_value = importValue)
...
# El peso del archivo supera el limite de github
