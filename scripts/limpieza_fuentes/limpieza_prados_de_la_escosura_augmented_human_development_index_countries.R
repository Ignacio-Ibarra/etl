...
sheets <- readxl::excel_sheets(xlsx_path)[-1] # Saco la primera hoja que es una portada.


limpiar_sheet <- function(sheet_name){
  
  anios_base <- c('1870', '1880', '1890', '1900', '1913', '1925',
                  '1929', '1933', '1938', '1950', '1955', '1960',
                  '1965', '1970', '1975', '1980', '1985', '1990',
                  '1995', '2000', '2005', '2010', '2015', '2020')
  
  cols <- c("pais_nombre", anios_base)
  
  regions <- c('Latin America and the Caribbean', 'Europe', 'Africa', 'Asia', 'Western Offshoots')
  
  str_variable_name <- readxl::read_excel(xlsx_path, sheet = sheet_name, range = "A1", col_names = F) %>% 
    pull() %>% 
    str_remove(., "Table \\d+\\. ")
  
  str_variable_id <- readxl::read_excel(xlsx_path, sheet = sheet_name, range = "A2", col_names = F) %>% 
    pull()
  
  sheet_data <- readxl::read_excel(xlsx_path, sheet = sheet_name, skip = 2, col_names = F)
  
  names(sheet_data) <- cols
  
  df <- sheet_data %>% 
    mutate(
      region = ifelse(pais_nombre %in% regions, pais_nombre, NA)
    ) %>% 
    fill(region, .direction = "down") %>%
    dplyr::filter(!(pais_nombre %in% regions)) %>% 
    pivot_longer(
      matches("\\d{4}"),
      names_to = 'anio',
...
df_raw <- purrr::map_dfr(sheets, limpiar_sheet)
...
nombres_ahdi <- df_raw %>% distinct(pais_nombre) %>% 
...
isos_encontrados <- nombres_ahdi %>% filter(esta) %>% pull(iso3)
paises_busqueda <- geonomenclador %>% filter(!(iso3 %in% isos_encontrados))
A <- paises_busqueda %>% pull(pais_nombre)
B <- nombres_ahdi %>% filter(!esta) %>% pull(pais_nombre)  # Nombres sucios
...
completar_automaticamente <- nombres_ahdi %>% 
  select(-esta) %>% 
  mutate(
...
df_clean <- df_raw %>% 
  mutate(
    ...
    region = traducir_regiones[region]
  ) %>% 
  select(-pais_nombre) %>% 
  left_join(geonomenclador, join_by(iso3)) %>% 
  select(anio, iso3, pais_nombre, region, variable_id, variable_name, valor)
