...
iso2_to_order <- WIPO.get_offices_tech(indicator = 100, rp_type = 13)$ipsOriginListSeq
...
paises <- df_raw %>% 
  ...
  left_join(geonomenclador, join_by(iso2))
...
df_clean <- bind_cols(paises, anios) %>% 
  ...
  select(-selectedOrigin_SeqOrder,-selectedOrigin) %>% 
  drop_na(valor)
