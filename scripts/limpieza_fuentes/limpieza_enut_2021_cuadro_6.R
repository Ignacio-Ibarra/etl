# Codigo de limpieza de datos de Encuesta Nacional de Uso del Tiempo 2021 Cuadro 6
...
enut_df_clean <-  enut_df %>% 
  ...
  fill(sexo) %>% 
  select(sexo, nivel_ed, `Trabajo total` = B, `En la ocupación` = C, `No remunerado` = D) %>% 
  pivot_longer(., !any_of(c("sexo", "nivel_ed")), names_to = "tipo_trabajo", values_to = "proporcion_dia") %>% 
  mutate(proporcion_dia = as.double(replace(proporcion_dia, proporcion_dia == "/// (1)", NA)),
...
actualizar_fuente_clean(id_fuente_clean = 20,
                        dir = tempdir())