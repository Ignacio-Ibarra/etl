...
dataset_metadata <- FAO.get_dataset_metadata(dataset_code) 
...
to_download <- bulkdownload_urls %>% 
  dplyr::filter(Type == "All")

url <- to_download$URL

zip_filename <- to_download$FileName

zip_destfile <- glue::glue("{tempdir()}/{zip_filename}")

download_filename <- str_replace(zip_filename, ".zip", ".csv")
...
actualizar_fuente_raw(id_fuente = 300,
                      url = url,
                      nombre = nombre,
                      fecha_actualizar = fecha_actualizar, 
                      path_raw = download_filename,
